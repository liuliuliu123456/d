...
It contains only the even components as the odd components have already been removed. The term $H_{0}(-z)X(-z)$ is an aliasing term, which is multiplied by $G_{0}(z)$. This aliasing term has to cancel the aliasing affect of  $G_{1}(z)H_{1}(-z)X(-z)$ from the other channel.
...
Now, the equation \ref{eqn:noalias} becomes:
...
This is a low-pass product filter. The high-pass product filer is ${P_1(z)=G_{1}(z)H_{1}(z)}$. The relationship between the terms $P_0$ and $P_1$ can be deduce by Equation \ref{eqn:antialiasing_cdn}:
...
\begin{enumerate}
	...
	\item Factor $P_0$ to find the values of $H_1$ and $G_1$.
\end{enumerate}

Let $p_0(n)$ be the impulse response corresponding to $P_0(Z)$
and the sequence $p(n)$ is obtained by shifting the $p_0(n)$ to the left side by $k$ samples to center it around the origin:
...
Subsituting Equation \ref{eqn:pin} into Equation \ref{eqn:updated_no_dist}, produces:

 \begin{equation}\label{eqn:piern} 
 {P(z) + P(-z) =  2}
 \end{equation}

Equation \ref{eqn:piern} implies that $P(Z)$ is a half-band filter, in which all even powers of $z$ in $P(z)$ are zero except for the one at origin.

One possibility to define the low-pass filter for $P_0(z)$ is to use the Daubechies function:

\begin{equation}\label{eqn:updated_no_distortion} 
{P_0(z) = (1 + z^{-1})^{2p}Q(z)}
\end{equation}

where $p$ can be any integer and $Q(z)$ be a polynomial of degree $(2p-2)$ is chosen to satisfy equation \ref{eqn:updated_no_dist}.
